...
class TestPage < Test::Unit::TestCase
  ...
  def test_json
    ...
    Bio::Graphics::Page.from_json(:json => @json_file, :outfile => @out_file )
    #test that the outfile exists and has a size > 0
    assert(File.size?(@out_file) > 0, "From TestJson, method test_file_exists: File doesn't exist, or is empty")
