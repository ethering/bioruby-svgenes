...
class SVGEE
  ...
  #for implementation of mouseover
  def pop_tag(a = {})
    %{<text x="#{a[:x]}" y="#{a[:y]}" visibility='hidden' >#{a[:text]}</text>
    <set attributeName="visibility" from="hidden" to="visible" begin="#{a[:glyphid]}.mouseover" end="#{a[:glyphid]}.mouseout"/>}
  end
