...
attr_reader :glyphs
  ...
  def self.scale(args)
    ...
    marks.each do |mark|
    #get the scale to display units in Kb/Mb/Gb if over 999
    x =   (mark.to_f - first_mark ).to_f * px_per_nt
    if mark >= 1000000000
      mark = mark.to_f/1000000000
      mark_st = (mark.round(2)).to_s+"Gb"
      mark = mark_st
    elsif mark >= 1000000
      mark = mark.to_f/1000000
      mark_st = (mark.round(2)).to_s+"Mb"
      mark = mark_st
    elsif mark >= 1000
      mark = mark.to_f/1000
      mark_st = (mark.round(2)).to_s+"Kb"
      mark = mark_st
    end
